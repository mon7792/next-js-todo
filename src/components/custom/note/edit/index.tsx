...
const EditNotesFormSchema = z.object({
...
type EditNotesFormType = z.infer<typeof EditNotesFormSchema>;
...
// EditNotesForm
export default function EditNotesForm({ note, onNoteCreate }: EditNotesFormProps) {
  const router = useRouter();

  // Define form.
  const form = useForm<EditNotesFormType>({
    resolver: zodResolver(EditNotesFormSchema),
    defaultValues: {
      title: note.title,
  ...
  const applyZodError = (form: any, err: z.ZodError): void => {
    ...
    Object.keys(flatten.fieldErrors).forEach((key) => {
      form.setError(key as keyof EditNotesFormType, {
        type: "manual",
  ...
  const onSubmit = async (values: EditNotesFormType) => {
    try {
